...
try:
    ...
    for rule in collected_rules.rules:#itero le sigma rule presenti
        ...
        tech_presente = False
        for techn in layer_fields["techniques"]: #aggiunto controllo per vedere se la Technique ID è già presente, in maniera tale da non sovrascrivere nulla sul layer
        ...
        if tech_presente == False:
            layer_fields["techniques"].append( #inserisco nel dizionario i campi estratti dalle Sigma rule, per poter creare poi il JSON layer di copertura delle rule
                {
